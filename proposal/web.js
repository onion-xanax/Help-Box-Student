    ...
    fetch(`/check_response/${currentSessionId}`)
        ...
        .then(data => {
            if (data.responded && data.admin_response) {
                ...
                addMessageToTerminal('IT hub~$', data.admin_response, 'admin');
                showNotification('Предложение принято', 'info');
...
function sendMessage() {
    ...
    fetch('/send_help_message', {
        ...
        .then(data => {
            if (data.success) {
                showNotification('Предложение отправлено администрации', 'success');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    showNotification('Поделитесь своими идеями по улучшению учебного процесса.', 'info', 5000);
});